...
const mode = ("mode" in window && window.mode as string) || "pvp";
...
function startGame() {
  ...
  board.cells.forEach(cell => cell.el.classList.remove(
    "cell-highlight",
    ...players.map(({ symbol }) => `cell-${symbol.toLowerCase()}`)
  ));
  game.start();
  mode == "o" && makeAIMove();
}