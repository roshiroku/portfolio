...
export function taskItem(task: Task) {
  ...
  const checkbox = taskCheckbox(task, () =>
    el.classList[checkbox.checked ? "add" : "remove"]("complete"));
  const editBtn = iconButton("pencil");
  const removeBtn = taskRemoveBtn(task, () => el.remove());
  const setButtons = (...buttons: HTMLElement[]) => {
    btnCol.innerHTML = "";
...
export function taskCheckbox(task: Task, onChange?: () => void) {
...
export function taskRemoveBtn(task: Task, onRemove?: () => void) {
